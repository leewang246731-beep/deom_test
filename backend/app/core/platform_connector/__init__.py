...
def get_platform_connector(shop_id: int, db: Session) -> PlatformConnector:
    ...
    - platform_type=taobao → TaobaoConnector (二期)
    """
    ...
    if shop.platform_type == "taobao":
        return TaobaoConnector(shop.app_key, shop.app_secret, shop.access_token)
    raise NotImplementedError(f"平台 {shop.platform_type} 暂未支持")


__all__ = [
    "PlatformConnector", "MockPlatformConnector", "TaobaoConnector", "V3Connector",
